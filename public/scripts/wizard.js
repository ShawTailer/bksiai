(function ($) {
  const ERRORS = {
    error404: {
      ...
      description:
        "Try putting index.php after your helpdesk installation's site url or If you are using apache, make sure that mod_rewrite module is enabled and AllowOverride directive for document root is set to All/FileInfo in your server's configuration file.",
    },
    error500: {
      title: "Something's bad happened with the server.",
      description:
        " Try again by clicking the back button or launch the wizard again by refershing the webpage or clicking the cancel button.",
    },
  };
  ...
  $(window).bind("load", function () {
    var UVDeskCommunityInstallSetupView = Backbone.View.extend({
      el: "#wizardContent",
      wizard: undefined,
      wizard_icons_notice_template: _.template(
        $("#wizardIcons-NoticeTemplate").html()
      ),
      installation_setup_template: _.template(
        $("#installationWizard-InstallSetupTemplate").html()
      ),
      installation_process_template: _.template(
        $("#installationWizard-InstallSetupTemplate-ProcessingItem").html()
      ),
      installation_successfull_template: _.template(
        $("#installationWizard-InstallationCompleteTemplate").html()
      ),
      events: {
      ...
      initialize: function (params) {
        ...
        this.wizard.reference_nodes.content.html(
          this.installation_setup_template()
        );
      },
      ...
      updateConfigurations: function () {
        ...
        (async () => {
          this.$el
            .find("#wizard-finalizeInstall")
            .html(
              this.installation_process_template({
                currentStep: "load-configurations",
              })
            );
          this.$el
            .find("#wizard-finalizeInstall .installation-progress-loader")
            .html(this.wizard.wizard_icons_loader_template());
          await $.post("./wizard/xhr/load/configurations").fail((response) => {
            if (response.status == 500) {
              this.$el
                .find(".wizard-svg-icon-failed-criteria-checklist")
                .html(this.wizard_icons_notice_template());
              this.$el
                .find("#error-message-bar")
                .html(
                  '</span> Issue can be resolved by simply <a href="https://www.uvdesk.com/en/blog/open-source-helpdesk-installation-on-ubuntu-uvdesk/" target="_blank"><p> enabling your <b>.env</b> file read/write permission</a> refresh the browser and try again.</p>'
                );
            }
          });
          ...
          this.$el
            ...
            .html(
              this.installation_process_template({
                currentStep: "load-website-prefixes",
              })
            );
          this.$el
            .find("#wizard-finalizeInstall .installation-progress-loader")
            .html(this.wizard.wizard_icons_loader_template());
          this.next(4);
          let websiteRoutes = await $.post(
            "./wizard/xhr/load/website-configure"
          ).fail((response) => {
            if (response.status == 500) {
              this.$el
                .find(".wizard-svg-icon-failed-criteria-checklist")
      ...
      redirectToWelcomePage: function () {
        this.$el.html(
          this.installation_successfull_template({
            prefixCollecton: this.wizard.prefix,
          })
        );
      },
      next: function ($i) {
    ...
    var UVDeskCommunityWebsiteConfigurationModel = Backbone.Model.extend({
        ...
        return new Promise((resolve, reject) => {
          $.get("./wizard/xhr/website-configure", (response) => {
            if (
              ...
              this.defaults["customer_panel_url"] =
                response.knowledgebasePrefix;
              resolve();
            } else {
      ...
      isProcedureCompleted: function (callback) {
        this.set("urlCollection", {
          "member-prefix": this.view.$el
            .find('input[name="memeberUrlPrefix"]')
            .val(),
          "customer-prefix": this.view.$el
            .find('input[name="customerUrlPrefix"]')
            .val(),
        });

        var wizard = this.view.wizard;
        wizard.reference_nodes.content
          .find("#wizardCTA-IterateInstallation")
          .prepend(
            '<span class="processing-request">' +
              wizard.wizard_icons_loader_template() +
              "</span>"
          );

        $.post(
          "./wizard/xhr/website-configure",
          this.get("urlCollection"),
          (response) => {
            if (
              typeof response.status != "undefined" &&
              true === response.status
          ...
          .always(function () {
            wizard.reference_nodes.content
              .find("#wizardCTA-IterateInstallation .processing-request")
              .remove();
          });
      },
    });
    ...
    var UVDeskCommunityWebsiteConfigurationView = Backbone.View.extend({
      ...
      wizard_website_configuration: _.template(
        $("#installationWizard-WebsiteConfigurationTemplate").html()
      ),
      initialize: function (params) {
        this.wizard = params.wizard;
        ...
        if (
          params.existingModel instanceof
      ...
      validateForm: _.debounce(function (event) {
        ...
        let memberPrefix = this.$el
          .find('input[name="memeberUrlPrefix"]')
          .val();
        let customerPrefix = this.$el
          .find('input[name="customerUrlPrefix"]')
          .val();
        ...
        if (memberPrefix == null || memberPrefix == "") {
          errorFlag = true;
          this.$el
            .find('.form-content input[name="memeberUrlPrefix"]')
            .after(
              "<span class='wizard-form-notice'>This field is mandatory</span>"
            );
        }
        ...
        if (customerPrefix == null || customerPrefix == "") {
          errorFlag = true;
          this.$el
            .find('.form-content input[name="customerUrlPrefix"]')
            .after(
              "<span class='wizard-form-notice'>This field is mandatory</span>"
            );
        }
        ...
        if (customerPrefix == memberPrefix) {
          ...
          this.$el
            .find('.form-content input[name="customerUrlPrefix"]')
            .after(
              "<span class='wizard-form-notice'>Both prefixes can not be same.</span>"
            );
        }
        ...
        if (!errorFlag) {
          ...
          if (!prefixTestRegex.test(memberPrefix)) {
            ...
            this.$el
              .find('.form-content input[name="memeberUrlPrefix"]')
              .after(
                "<span class='wizard-form-notice'>Only letters and numbers are allowed</span>"
              );
          }
    ...
    var UVDeskCommunityAccountConfigurationView = Backbone.View.extend({
      ...
      model: undefined,
      wizard: undefined,
      account_settings_template: _.template(
        $("#installationWizard-AccountConfigurationTemplate").html()
      ),
      events: {
      ...
      validateForm: _.debounce(function (event) {
        ...
        let passwordRegix =
          /^(?=(.*[a-zA-Z].*){2,})(?=.*\d.*)(?=.*\W.*)[a-zA-Z0-9\S]{8,}$/;

        let user = {
          name: this.$el.find('input[name="name"]').val(),
          email: this.$el.find('input[name="email"]').val(),
          password: this.$el.find('input[name="password"]').val(),
          confirmPassword: this.$el
            .find('input[name="confirm_password"]')
            .val(),
        };

        let selectedElement = this.$el.find(event.target).parent();
        this.$el.find(".wizard-form-notice")
          ? this.$el.find(".wizard-form-notice").remove()
          : "";
        ...
        if (!errorFlag && user.name) {
          if (!nameRegex.test(user.name)) {
            ...
            this.$el
              ...
              .parent()
              .append("<span class='wizard-form-notice'>Invalid Name</span>");
          }
        }
        ...
        if (!errorFlag && user.email !== "") {
          if (!emailRegex.test(user.email)) {
            ...
            this.$el
              ...
              .parent()
              .append("<span class='wizard-form-notice'>Invalid Email</span>");
          }
        }
        ...
        if (user.password.length > 0 && !passwordRegix.test(user.password)) {
          ...
          this.$el
            ...
            .parent()
            .append(
              "<span class='wizard-form-notice'>Password must contain minimum 8 character length, at least two letters (not case sensitive), one number, one special character(space is not allowed).</span>"
            );
        }
        ...
        if (
          ...
          this.$el
            ...
            .parent()
            .append(
              "<span class='wizard-form-notice'>Password does not match.</span>"
            );
        }

        if (
          (!errorFlag && (user.name == null || user.name == "")) ||
          user.email == null ||
          user.email == "" ||
          user.password == null ||
          user.password == "" ||
    ...
    var UVDeskCommunityDatabaseConfigurationModel = Backbone.Model.extend({
      ...
      isProcedureCompleted: function (callback) {
        this.set("credentials", {
          ...
          createDatabase: this.view.$el
            .find('input[name="createDatabase"]')
            .prop("checked")
            ? 1
            : 0,
        });
        ...
        $.post(
          ...
          function (response) {
            if (
              ...
              this.view.$el
                .find('.form-content input[name="createDatabase"]')
                .parents(".form-content")
                .append(
                  "<span id='wizard-error-id' class='wizard-form-notice'>Details are incorrect ! Connection not established.</span>"
                );
              wizard.disableNextStep();
            }
          }.bind(this)
    ...
    var UVDeskCommunityDatabaseConfigurationView = Backbone.View.extend({
      ...
      model: undefined,
      wizard: undefined,
      database_configuration_template: _.template(
        $("#installationWizard-DatabaseConfigurationTemplate").html()
      ),
      events: {
        "keyup #wizard-configureDatabase .form-content input": "validateForm",
        "change #wizard-configureDatabase .form-content input[type='checkbox']":
          "validateForm",
      },
      initialize: function (params) {
        if (
        ...
        this.$el.html(
          this.database_configuration_template(this.model.attributes)
        );
      },
      validateForm: _.debounce(function (event) {
        let errorFlag = false;
        let mandatoryFieldsCollection = [
          "serverName",
          "username",
          "password",
          "database",
        ...
        let selectedElement = this.$el.find(event.target).parent();
        selectedElement.find(".wizard-form-notice")
          ? selectedElement.find(".wizard-form-notice").remove()
          : "";
    ...
    var UVDeskCommunitySystemRequirementsModel = Backbone.Model.extend({
        ...
        $.post("./wizard/xhr/check-requirements", postData, (response) => {
          ...
          .fail((jqXHR, textStatus, errorThrown) => {
            this.set("php-version", {
              status: false,
              message: ERRORS.hasOwnProperty("error" + jqXHR.status)
                ? ERRORS["error" + jqXHR.status].title
                : "An unexpected error occurred during the PHP version verification process",
              description: ERRORS.hasOwnProperty("error" + jqXHR.status)
                ? ERRORS["error" + jqXHR.status].description
        ...
        $.post("./wizard/xhr/check-requirements", postData, (response) => {
          ...
          .fail((jqXHR, textStatus, errorThrown) => {
            this.set("php-maximum-execution", {
              ...
              message: ERRORS.hasOwnProperty("error" + jqXHR.status)
                ? ERRORS["error" + jqXHR.status].title
                : "An unexpected error occurred during the Maximum execution time verification process",
              description: ERRORS.hasOwnProperty("error" + jqXHR.status)
                ? ERRORS["error" + jqXHR.status].description
                : 'Maximum Execution Time  </span><p>Need to resolve this issue can be done by reading this blog link:<a href="https: //www.simplified.guide/php/increase-max-execution-time" target="_blank">How to resolve PHP mailparse extension</a></p>',
            });
          })
          .always(() => {
      ...
      evaluateOverallRequirements: function () {
        if (false == this.get("php-version").status) {
          ...
          this.get("php-configfiles-permission").hasOwnProperty("configfiles")
        ) {
    ...
    var UVDeskCommunitySystemRequirementsView = Backbone.View.extend({
      ...
      wizard: undefined,
      events: {
        "click .PHPExtensions-toggle-details, .PHPVersion-toggle-details, .PHPPermissionEnvfile-toggle-details, .PHPExeTime-toggle-details, .PHPPermissionConfigfiles-toggle-details":
          function (e) {
            // show and hide extension details
            const currentElement = Backbone.$(e.currentTarget);
            currentElement
              .parents('[class*="info-container"]')
              .siblings(".systemCriteria-Details")
              .toggle();
      ...
      wizard_icons_loader_template: _.template(
        $("#wizardIcons-LoaderTemplate").html()
      ),
      wizard_icons_success_template: _.template(
        $("#wizardIcons-SuccessTemplate").html()
      ),
      wizard_icons_notice_template: _.template(
        $("#wizardIcons-NoticeTemplate").html()
      ),
      wizard_system_requirements_template: _.template(
        $("#installationWizard-SystemRequirementsTemplate").html()
      ),
      wizard_system_requirements_php_ver_template: _.template(
        $("#installationWizard-SystemRequirementsTemplate-PHPVersion").html()
      ),
      wizard_system_requirements_php_ext_template: _.template(
        $("#installationWizard-SystemRequirementsTemplate-PHPExtensions").html()
      ),
      wizard_system_requirements_php_exe_template: _.template(
        $("#installationWizard-SystemRequirementsTemplate-PHPExecution").html()
      ),
      wizard_system_requirements_php_env_template: _.template(
        $("#installationWizard-SystemRequirementsTemplate-PHPPermission").html()
      ),
      wizard_system_requirements_php_config_template: _.template(
        $(
          "#installationWizard-SystemRequirementsTemplate-PHPPermissionConfigfiles"
        ).html()
      ),
      initialize: function (params) {
        this.wizard = params.wizard;
        ...
        // Set reference nodes
        this.reference_nodes.version = this.$el.find(
          "#systemCriteria-PHPVersion"
        );
        this.reference_nodes.extension = this.$el.find(
          "#systemCriteria-PHPExtensions"
        );

        this.reference_nodes.execution = this.$el.find(
          "#systemCriteria-PHPExecution"
        );
        this.reference_nodes.permission = this.$el.find(
          "#systemCriteria-PHPPermission"
        );

        this.reference_nodes.Configfiles = this.$el.find(
          "#systemCriteria-PHPPermissionConfigfiles"
        );
      ...
      renderPHPVersion: function (status) {
        this.reference_nodes.version.html(
          this.wizard_system_requirements_php_ver_template(
            this.model.get("php-version")
          )
        );
        this.reference_nodes.version.find(".PHPVersion-toggle-details").hide();

        if (false == this.model.get("fetch")) {
          this.reference_nodes.version
            .find(".wizard-svg-icon-criteria-checklist")
            .html(this.wizard_icons_loader_template());
          this.reference_nodes.version
            .find("label")
            .html("Checking currently enabled PHP version");
        } else {
          if (true === this.model.get("php-version").status) {
            this.reference_nodes.version
              .find(".wizard-svg-icon-criteria-checklist")
              .html(this.wizard_icons_success_template());
            this.reference_nodes.version
              .find("label")
              .html(this.model.get("php-version").message);
          } else {
            this.reference_nodes.version
              .find(".wizard-svg-icon-criteria-checklist")
              .html(this.wizard_icons_notice_template());
            ...
            if (this.model.get("php-version").hasOwnProperty("description")) {
              this.reference_nodes.version
                .find(".PHPVersion-toggle-details")
                .show();
            }
            this.reference_nodes.version
              .find(".systemCriteria-Details")
              .addClass("systemCriteria-Info-Message");
            this.reference_nodes.version
              .find("#systemCriteria-PHPVersion-Details")
              .html(this.model.get("php-version").description);
          }
        }
      },
      renderPHPExtensionsCriteria: function (status) {
        this.reference_nodes.extension.html(
          this.wizard_system_requirements_php_ext_template(
            this.model.get("php-extensions")
          )
        );

        if (false == this.model.get("fetch")) {
          this.reference_nodes.extension
            .find(".wizard-svg-icon-criteria-checklist")
            .html(this.wizard_icons_loader_template());
          this.reference_nodes.extension
            .find("label")
            .html("Checking currently enabled PHP extensions");
        } else if (
          this.model.get("php-extensions").hasOwnProperty("extensions")
        ) {
          var activeExtensionCount = 0;
          var extensionCount =
            this.model.get("php-extensions").extensions.length;
          ...
          this.model.get("php-extensions").extensions.forEach((extension) => {
            let currentExtensionName = Object.keys(extension)[0];
            let currentExtensionTemplateInfo =
              this.reference_nodes.extension.find(
                "#" + currentExtensionName + "-info"
              );
            if (extension[currentExtensionName]) {
              activeExtensionCount++;
              var currentExtensionIconStatus =
                this.wizard_icons_success_template();
              var currentExtensionTextStatus =
                "<span class='extension_name'>" +
                currentExtensionName +
                "</span> extension is currently active.";
            } else {
              var currentExtensionIconStatus =
                this.wizard_icons_notice_template();
              if (currentExtensionName == "imap") {
                var currentExtensionTextStatus =
                  "<span class='extension_name'> PHP " +
                  currentExtensionName +
                  " extension </span><p>Need to resolve this issue can be done by reading this blog link:<a href='https://www.php.net/manual/en/imap.setup.php' target='_blank'>How to resolve PHP imap extension</a></p>";
              } else if (currentExtensionName == "mailparse") {
                var currentExtensionTextStatus =
                  "<span class='extension_name'> PHP " +
                  currentExtensionName +
                  " extension </span><p>Need to resolve this issue can be done by reading this blog link:<a href='https://www.php.net/manual/en/book.mailparse.php' target='_blank'>How to resolve PHP mailparse extension</a></p>";
              } else {
                var currentExtensionTextStatus =
                  "<span class='extension_name'>" +
                  currentExtensionName +
                  "extension is currently inactive</span>";
              }
            }

            currentExtensionTemplateInfo
              .find(".wizard-svg-icon-criteria-checklist")
              .html(currentExtensionIconStatus);
            currentExtensionTemplateInfo
              .find("label")
              .html(currentExtensionTextStatus);
          });
          ...
          extension_info
            .find(".wizard-svg-icon-extension-criteria-checklist")
            .html(overallExtensionStatus);
          extension_info
            .find(".extension-criteria-label")
            .html(
              "You meet " +
                activeExtensionCount +
                " out of " +
          ...
          this.reference_nodes.extension
            .find(".extension-criteria-label")
            .html(this.model.get("php-extensions").message);
          this.reference_nodes.extension
            .find("#systemCriteria-PHPExtensions-Details")
            .html(this.model.get("php-extensions").description);
          this.reference_nodes.extension
            .find("#systemCriteria-PHPExtensions-Details")
            .addClass("systemCriteria-Info-Message");
        }
      ...
      renderPHPmaximumexecution: function (status) {
        this.reference_nodes.execution.html(
          this.wizard_system_requirements_php_exe_template(
            this.model.get("php-maximum-execution")
          )
        );
        this.reference_nodes.execution
          .find(".PHPExeTime-toggle-details")
          .hide();
        if (false == this.model.get("fetch")) {
          this.reference_nodes.execution
            .find(".wizard-svg-icon-execution-criteria-checklist")
            .html(this.wizard_icons_loader_template());
          this.reference_nodes.execution
            .find("label")
            .html("Checking maximum execution time");
        } else {
          if (true === this.model.get("php-maximum-execution").status) {
            ...
            this.reference_nodes.execution
              .find("label")
              .html(this.model.get("php-maximum-execution").message);
          } else {
            this.reference_nodes.execution
              .find(".wizard-svg-icon-execution-criteria-checklist")
              .html(this.wizard_icons_notice_template());
            ...
            this.reference_nodes.execution
              ...
              .addClass("systemCriteria-Info-Message");
            this.reference_nodes.execution
              .find("#systemCriteria-PHPExecution-Details")
              .html(this.model.get("php-maximum-execution").description);
          }
        }
      },
      renderEnvFilePermission: function (status) {
        this.reference_nodes.permission.html(
          this.wizard_system_requirements_php_env_template(
            this.model.get("php-envfile-permission")
          )
        );
        this.reference_nodes.permission
          .find(".PHPPermissionEnvfile-toggle-details")
          .hide();

        if (false == this.model.get("fetch")) {
          this.reference_nodes.permission
            .find(".wizard-svg-icon-permissionEnvfile-criteria-checklist")
            .html(this.wizard_icons_loader_template());
          this.reference_nodes.permission
            .find("label")
            .html("Checking currently enabled .env file");
        } else {
          if (true === this.model.get("php-envfile-permission").status) {
            ...
            this.reference_nodes.permission
              .find("label")
              .html(this.model.get("php-envfile-permission").message);
          } else {
            this.reference_nodes.permission
              .find(".wizard-svg-icon-permissionEnvfile-criteria-checklist")
              .html(this.wizard_icons_notice_template());
            ...
            this.reference_nodes.permission
              ...
              .addClass("systemCriteria-Info-Message");
            this.reference_nodes.permission
              .find("#systemCriteria-PHPPermission-Details")
              .html(this.model.get("php-envfile-permission").description);
          }
        }
      },
      renderConfigFilesPermission: function (status) {
        this.reference_nodes.Configfiles.html(
          this.wizard_system_requirements_php_config_template(
            this.model.get("php-configfiles-permission")
          )
        );
        this.reference_nodes.Configfiles.find(
          ".PHPPermissionConfigfiles-error-message"
        ).hide();
        if (false == this.model.get("fetch")) {
          this.reference_nodes.Configfiles.find(
            ".wizard-svg-icon-permissionConfigfiles-criteria-checklist"
          ).html(this.wizard_icons_loader_template());
          this.reference_nodes.Configfiles.find("label").html(
            "Checking currently enabled Config-files"
          );
        } else if (
          this.model
            .get("php-configfiles-permission")
            .hasOwnProperty("configfiles")
        ) {
          var activeconfigfileCount = 0;
          var configfileCount = this.model.get("php-configfiles-permission")
            .configfiles.length;
          // count the active extensions and set each extension with it's status in the extension list
          this.model
            .get("php-configfiles-permission")
            .configfiles.forEach((configfile) => {
              ...
              let currentconfigfileTemplateInfo =
                this.reference_nodes.Configfiles.find(
                  "#" + currentconfigfileName + "-info"
                );
              if (configfile[currentconfigfileName]) {
                activeconfigfileCount++;
                var currentconfigfileIconStatus =
                  this.wizard_icons_success_template();
                var currentconfigfileTextStatus =
                  "<span class='configfiles_name'>" +
                  currentconfigfileName +
                  ".yaml </span> read/write file permission is enabled.";
              } else {
                var currentconfigfileIconStatus =
                  this.wizard_icons_notice_template();
                if (currentconfigfileName == "uvdesk") {
                  var currentconfigfileTextStatus =
                    "<span class='configfiles_name'> " +
                    currentconfigfileName +
                    ".yaml  read/write file permission is disabled </span>";
                } else {
                  var currentconfigfileTextStatus =
                    "<span class='configfiles_name'>" +
                    currentconfigfileName +
                ...
                this.reference_nodes.Configfiles.find(
                  ...
                  this.model.get("php-configfiles-permission").description
                );
              }

              currentconfigfileTemplateInfo
                .find(".wizard-svg-icon-criteria-checklist")
                .html(currentconfigfileIconStatus);
              currentconfigfileTemplateInfo
                .find("label")
                .html(currentconfigfileTextStatus);
            });
          // set overall response with the count of active extensions
          let configfile_info = this.$el.find(
            "#systemCriteria-PHPPermissionConfigfiles"
          );
          ...
          configfile_info
            .find(".wizard-svg-icon-permissionConfigfiles-criteria-checklist")
            .html(overallconfigfileStatus);
          configfile_info
            .find(".permissionConfigfiles-criteria-label")
            .html(
              "You meet " +
                activeconfigfileCount +
                " out of " +
          ...
          this.reference_nodes.configfiles
            .find(".wizard-svg-icon-permissionConfigfiles-criteria-checklist")
            .html(this.wizard_icons_notice_template());
          this.reference_nodes.configfiles
            .find(".permissionConfigfiles-criteria-label")
            .html(this.model.get("php-configfiles-permission").message);
          this.reference_nodes.configfiles
            .find("#systemCriteria-PHPPermissionConfigfiles-Details")
            .html(this.model.get("php-configfiles-permission").description);
          this.reference_nodes.configfiles
            .find("#systemCriteria-PHPPermissionConfigfiles-Details")
            .addClass("systemCriteria-Info-Message");
        }
    ...
    var UVDeskCommunityInstallationWizardView = Backbone.View.extend({
      ...
      wizard_default_header_template: _.template(
        $("#installationWizard-DefaultHeaderTemplate").html()
      ),
      wizard_default_content_template: _.template(
        $("#installationWizard-DefaultContentTemplate").html()
      ),
      wizard_setup_component_template: _.template(
        $("#installationWizard-SetupTemplate").html()
      ),
      events: {
        "click #wizardCTA-StartInstallation": function () {
          ...
          this.reference_nodes.content.html(
            this.wizard_setup_component_template()
          );
        ...
        "click #wizardCTA-IterateInstallation": function () {
          if (typeof this.activeSetupProcedure != "undefined") {
            ...
            this.activeSetupProcedure.model.isProcedureCompleted(
              function ({ wizard, model }) {
                ...
                if (typeof activeInstanceIndex != "undefined") {
                  ...
                  if (
                    typeof wizard.timeline[activeInstanceIndex + 1] !==
                    "undefined"
                  ) {
                    wizard.router.navigate(
                      wizard.timeline[activeInstanceIndex + 1].path,
                      { trigger: true }
                    );
                  }
                }
          ...
          this.timeline.filter((values, index) => {
            if (values.isActive && this.timeline[index - 1]) {
              if (this.timeline[index].path == "configure-database") {
                ...
                this.timeline[index].isActive = false;
                this.router.navigate(this.timeline[index - 1].path, {
                  trigger: true,
                });
              }
      ...
      iterateInstallationSteps: function (iteration) {
        if ("welcome" === iteration) {
          ...
          if (!this.enabled) {
            ...
            this.timeline.every(
              function (installationStep, index) {
                if (
                  iteration == installationStep.path &&
                  typeof installationStep.view != "undefined"
                ) {
      ...
      renderWizard: function () {
        ...
        this.reference_nodes.content.html(
          this.wizard_default_content_template()
        );
      },
      renderWizardInstallationStep: function ({
        view: InstallationWizardTemplateView,
        model: InstallationWizardTemplateModel,
      }) {
        this.disableNextStep();
        this.reference_nodes.content.find("#wizardSetup").empty();
        this.activeSetupProcedure = new InstallationWizardTemplateView({
          wizard: this,
          existingModel: InstallationWizardTemplateModel,
        });
      },
